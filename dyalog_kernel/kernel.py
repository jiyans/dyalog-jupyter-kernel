...
class DyalogKernel(Kernel):
    ...
    language_info = {"name": "APL", "mimetype": "text/apl", "file_extension": ".apl"}
    ...
    def __init__(self, **kwargs):
        ...
        if DYALOG_HOST == "127.0.0.1":
            ...
            while True:
                try:
                    ...
                    sock.bind(
                        (str(DYALOG_HOST).strip(), 0)
                    )  # Port is available since I can bind
                    # Get the port number
        ...
        if DYALOG_HOST == "127.0.0.1":
            if sys.platform.lower().startswith("win"):
                ...
                CloseKey(dyalogKey)
                CloseKey(lastKey)
                self.dyalog_subprocess = subprocess.Popen(
                    [
                        dyalogPath,
                        "RIDE_SPAWNED=1",
                        "DYALOGQUIETUCMDBUILD=1",
                        "Dyalog_LineEditor_Mode=1",
                        "RIDE_INIT=SERVE::" + str(self._port).strip(),
                        "LOG_FILE_INUSE=0",
                        "DYALOGJUPYFOLDER="
                ...
                dyalog_env["DYALOGJUPYFOLDER"] = os.path.dirname(
                ...
                if shutil.which("mapl"):
                    ...
                    for d in sorted(os.listdir("/Applications")):
                        if re.match(r"^Dyalog-\d+\.\d+\.app$", d):
                            dyalog = (
                                "/Applications/" + d + "/Contents/Resources/Dyalog/mapl"
                            )
    ...
    def do_execute(
        ...
        if not silent:
            if self.connected:
                lines = code.split("\n")
                match = re.search(
                    r"^%suspend\s+(\w+)$", lines[0].lower(), re.IGNORECASE
                )
                if match:
                ...
                try:
                    ...
                    for line in lines:
                        ...
                        while len(dq) > 0 or not PROMPT_AVAILABLE:
                            ...
                            if received[0] == "AppendSessionOutput":
                                if (not PROMPT_AVAILABLE) and (
                                    received[1].get("type") != 14
                                ):
                                    data_collection = data_collection + received[1].get(
                                        "result"
                                    )
                    ...
                    if pt == 3:
                        self.ride_send(
                            ["WeakInterrupt", {}]
                        )  # should be StrongInterrupt, which is not working
                        if not SUSPEND:
                            self.execute_line("→\n")
    ...
    def define_function(self, lines):
        ...
        if re.match("^\\s*:namespace|:class|:interface", lines[0].lower()):
            ...
            self.execute_line(
                "{''≢0⍴r←⎕FX ⍵:511 ⎕SIGNAL⍨'DEFN ERROR: Issue on line ',⍕r}⎕SE.Dyalog.ipyFn\n"
            )
